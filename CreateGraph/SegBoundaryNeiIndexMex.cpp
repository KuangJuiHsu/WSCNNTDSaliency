...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    /* Variables */
    int NumRows, NumCols, Row, Col, i, Shift[4][2], TempRow, TempCol, MaxLength, CurrentIndex, NeiIndex;
    mxLogical *Segmap;
    double  *RowIndex, *ColIndex, *NeighborRowIndex, *NeighborColIndex, *Count;
    
    /* Input */
    Segmap = (bool *) mxGetData(prhs[0]);
    ...
    for(Row = 0; Row < NumRows; Row ++)
    {
        for(Col = 0; Col < NumCols; Col ++)
        {
            ...
            for(i = 0; i < 4; i++)
            {
                ...
                {
                    NeiIndex = TempRow + NumRows * TempCol;
                    if(Segmap[CurrentIndex] != Segmap[NeiIndex])
                    {
