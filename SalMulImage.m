classdef SalMulImage< dagnn.ElementWise
    ...
    methods
        function outputs = forward(obj, inputs, params)
            ...
            obj.ObjImaID = inputs{5} == 1;
            obj.NumObjImgs = sum(obj.ObjImaID);
            ...
            OriginalObjImg = bsxfun(@plus, inputs{3}(:,:,:,obj.ObjImaID), inputs{4});
            ObjNegative = bsxfun(@minus, bsxfun(@times, OriginalObjImg, inputs{1}(:,:,:, obj.ObjImaID)), inputs{4});
            ObjPositive = bsxfun(@minus, bsxfun(@times, OriginalObjImg, inputs{2}(:,:,:, obj.ObjImaID)), inputs{4});
            ...
            outputs{4} = inputs{2}(:,:,:,obj.ObjImaID);
        end
        ...
        function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
            ...
            derInputs{1}(:,:,:,obj.ObjImaID) = sum(derOutputsObjNegative .* OriginalObjImg,3);
            derInputs{2}(:,:,:,obj.ObjImaID) = sum(derOutputsObjPositive .* OriginalObjImg,3);
            derInputs{2}(:,:,:,obj.BGImgID) = derOutputs{3};
            derInputs{2}(:,:,:,obj.ObjImaID) = ...
                derInputs{2}(:,:,:,obj.ObjImaID)...
               + derOutputs{4};
            derInputs{3} = 1;
            derInputs{4} = 1;
