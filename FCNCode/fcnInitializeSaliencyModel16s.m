function net = fcnInitializeSaliencyModel16s(net)
...
filters = single(bilinear_u(4, 1, NumClasses)) ;
...
net.addLayer('skip4', ...
     dagnn.Conv('size', [1 1 512 NumClasses], 'pad', 0), ...
     'x24', 'x40', {'skip4f','skip4b'});

f = net.getParamIndex('skip4f') ;
net.params(f).value = zeros(1, 1, 512, NumClasses, 'single') ;
net.params(f).learningRate = 0.1 ;
net.params(f).weightDecay = 1 ;

f = net.getParamIndex('skip4b') ;
net.params(f).value = zeros(1, 1, NumClasses, 'single') ;
...
filters = single(bilinear_u(32, NumClasses, NumClasses)) ;
