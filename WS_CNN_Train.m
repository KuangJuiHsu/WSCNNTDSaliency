function [net,stats] = WS_CNN_Train(net, imdb, getBatch, varargin)
...
DataID = opts.(mode);
DataImgageLabel = state.imdb.ImageLabelList(DataID);
PositiveDataID = DataID(DataImgageLabel == 1);
NegativeDataID = DataID(DataImgageLabel == 0);
NumPositiveData = numel(PositiveDataID);
NumNegativeData = numel(NegativeDataID);
NumData = NumPositiveData + NumNegativeData;
assert(NumPositiveData == NumNegativeData, 'Currently Assume NumPositiveData = NumNegativeData!!!')
subset = zeros(1, NumData);
subset(1:2:end) = PositiveDataID(randperm(NumPositiveData));
subset(2:2:end) = NegativeDataID(randperm(NumPositiveData));
